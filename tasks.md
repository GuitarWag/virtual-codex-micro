...
- [ ] [T-VCMPLAN1-006] **Scaffold Xcode project and build config** `priority:high` `assignee:claude` `tags:m1,panel` `due:2026-08-05`
  > SwiftUI macOS app target, minimum macOS 14, LSUIElement so no Dock icon or menu bar, no storyboard. Set up SwiftFormat/SwiftLint, a debug scheme, and one smoke test target that launches the app and asserts the panel exists. Keep the dependency list empty at this stage; nothing here needs a package yet.
  > Created: 2026-07-26T00:00:00.000Z | Updated: 2026-07-26T00:00:00.000Z
...
- [ ] [T-VCMPLAN1-021] **Claude Code adapter: observe path via hooks** `priority:critical` `assignee:claude` `tags:m2,backend,state` `due:2026-09-06`
  > Local event receiver plus an idempotent installer that adds our hook entries to ~/.claude/settings.json without touching or duplicating existing user hooks, with explicit consent and a clean uninstall. Maps received events to normalized states using the table produced by the M0 hook spike. Primary state source for sessions the user started themselves.
  > Created: 2026-07-26T00:00:00.000Z | Updated: 2026-07-26T00:00:00.000Z

- [ ] [T-VCMPLAN1-022] **Claude Code adapter: transcript tailing fallback** `priority:medium` `assignee:claude` `tags:m2,backend,state` `due:2026-09-09`
  > FSEvents watcher over ~/.claude/projects that discovers and states sessions which predate hook installation, so opening the panel mid-session shows something truthful. Yields lower-confidence states, and confidence must be visible to the engine so hook data always wins on conflict. Ship only if the M0 accuracy spike cleared it.
  > Created: 2026-07-26T00:00:00.000Z | Updated: 2026-07-26T00:00:00.000Z
...
- [ ] [T-VCMPLAN1-023] **Claude Code adapter: owned sessions under PTY** `priority:critical` `assignee:claude` `tags:m2,backend` `due:2026-09-13`
  > Spawn and supervise sessions the app owns, using the approach validated in the M0 PTY spike. Dispatch accept, reject, new session and effort changes through controlled stdin. Handle lifecycle properly: child crash, user quitting the app, orphaned processes on hard kill. This is the only path where the command cluster is fully live, so its reliability defines the product's action story.
  > Created: 2026-07-26T00:00:00.000Z | Updated: 2026-07-26T00:00:00.000Z

- [ ] [T-VCMPLAN1-024] **Focus action for bound sessions** `priority:high` `assignee:claude` `tags:m2,backend` `due:2026-09-16`
  > Single click on an agent key raises the window and tab that owns the session, per the emulator matrix from the M0 focus spike, including tmux window selection. Degrade honestly: where the owning emulator cannot be targeted, say so on hover instead of clicking to no effect. The one action available on every session type, so it carries more weight than its size suggests.
  > Created: 2026-07-26T00:00:00.000Z | Updated: 2026-07-26T00:00:00.000Z
